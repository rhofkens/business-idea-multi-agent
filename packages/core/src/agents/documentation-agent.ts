...
function createDocumentationAgent(executionContext: string) {
  return AgentFactory.createAgent({
    ...
    provider: configService.documentationModelSpec.provider,
    model: configService.documentationModelSpec.model,
    enableWebSearch: false // Documentation doesn't use web search
  }, configService.getApiKeyForProvider(configService.documentationModelSpec.provider));
}
...
- Include a placeholder for the date: "Generated On: {{DATE_PLACEHOLDER}}"
...
`;
  ...
  try {
    ...
    let introduction = result.finalOutput ?? '';
    
    // Replace any date generated by the LLM with the actual current date
    // This handles various date formats the LLM might generate
    const now = new Date();
    const formattedDate = now.toISOString().replace('T', ' ').substring(0, 19) + ' UTC';
    
    // Replace common date patterns with the actual date
    introduction = introduction.replace(
      /Generated On:.*(?:\n|$)/i,
      `Generated On: ${formattedDate}\n`
    );
    
    // Also handle the placeholder if the LLM follows instructions
    introduction = introduction.replace('{{DATE_PLACEHOLDER}}', formattedDate);
